...
QCatGrayBrushPixBufferItem::QCatGrayBrushPixBufferItem(QGraphicsItem *parent)
...
QCatGrayBrushPixBufferItem::~QCatGrayBrushPixBufferItem()
{
...
void QCatGrayBrushPixBufferItem::Clear()
{
    m_pBufferBrush->fill(Qt::transparent);
    this->update();
}

void QCatGrayBrushPixBufferItem::DrawToBuffer(QCatGrayBrushObject *object)
{
    if(m_pBufferPainter != nullptr)
    {
...
void QCatGrayBrushPixBufferItem::InitSizeRect(QRectF size)
{
...
void QCatGrayBrushPixBufferItem::UpdateSizeRect(QRectF size)
{
...
QRectF QCatGrayBrushPixBufferItem::boundingRect() const
{
    return m_ySizeRect;
}

void QCatGrayBrushPixBufferItem::InitProperty()
{
    /*
     * 缓存是在绘制设备级别的设备坐标中启用的。
...
void QCatGrayBrushPixBufferItem::InitBrush()
{
    /*
